...
class AesEcbDecryptor:
    def __init__(self, key: bytes):
        ...
        self._set_property_string(self._alg, BCRYPT_CHAINING_MODE, BCRYPT_CHAIN_MODE_ECB)
        object_length = self._get_property_u32(self._alg, BCRYPT_OBJECT_LENGTH)
        block_length = self._get_property_u32(self._alg, BCRYPT_BLOCK_LENGTH)
        if block_length != 16:
            raise NativeCryptoError(f"Unexpected AES block length: {block_length}")
    ...
    def decrypt(self, ciphertext: bytes) -> bytes:
        if len(ciphertext) % 16 != 0:
            raise ValueError("AES-ECB ciphertext length must be a multiple of 16.")
    ...
    def __enter__(self) -> "AesEcbDecryptor":
        return self
